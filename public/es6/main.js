    ...
    startWebSocket(){
        ...
        this.server.onmessage = (e) => {
            ...
            if(typeof e.data == 'string'){
                ...
                if(x[0] == 8){// laser
                    var laserData = new Int16Array(d);
                    var lasers = [];
                    var singleLaser = [];
                    for(var k=2; k<laserData.length; k++){
                        singleLaser.push(laserData[k]);
                        if(singleLaser.length == laserData[1]){
                            singleLaser.push(Date.now());
                            lasers.push(singleLaser);
                            singleLaser = [];
                        }
                    }
                    Game.data.lasers = Game.data.lasers.concat(lasers);
                    Game.draw.lasers();
                    console.log(Game.data.lasers);
                }
